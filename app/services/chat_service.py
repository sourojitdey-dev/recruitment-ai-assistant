...
def is_casual_message(message: str) -> bool:
    ...
    return cleaned in CASUAL_MESSAGES
...
def answer_user_query(
    ...
    if not is_safe:
        return {
            "answer": safety_err
            or "Query rejected by platform safety policy.",
    ...
    # 3. Check whether this is a simple casual message
    if is_casual_message(message):
        ...
        prompt_messages = [
            {
                "role": "system",
                "content": (
                    "You are a friendly AI recruitment and career assistant. "
                    "Respond naturally and briefly to casual conversation. "
                    "Do not search documents or use retrieved context for "
        ...
        try:
            ...
            raw_response = (
                "Could not generate AI response at this moment: "
                f"{e}"
            )
    ...
    # 4. Retrieve authorized context snippets
    ...
    # 10. Update session title if first turn
    if len(history) == 0:
        session.title = message[:40] + (
            "..." if len(message) > 40 else ""
        )
